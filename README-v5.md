...
## v8 界面与学习交互更新（2026-09-19）

本次按手机端使用反馈加入 5 项交互：

- 右下角新增 **“⌖ 当前句”** 悬浮按钮；播放过程中可一键回到正在播放的句子，并把该句滚动到屏幕中间。即使已经切到别的章节/页，也会自动回到对应位置。
- 每个句子新增 **“▾ 讲解”** 按钮。展开后显示语法结构、时态/语气、语态、语法要点和重点词语。讲解逻辑全部内置在 `index.html` 中，本地运行和 PWA 离线状态都可使用，不依赖在线 API。
- 学习进度移动到标题栏右侧、安装/主题按钮左边，只保留简洁的 **“已学 n / 2000”** 信息；搜索区单独占一行。
- 屏幕左右两侧新增长窄形 **上一页 / 下一页** 按钮。它们只切换当前分类里的列表页，不会停止或改变当前播放。
- 句子内容区域左右滑动改为 **切换章节**（问候与礼貌、自我介绍……），不再把左右滑动解释成上一句/下一句；当前音频继续播放。

另外将 Service Worker 缓存键升级到 `fr2000-v8-20260919-ui`，避免已安装 PWA 继续使用旧版 `index.html`。


## v9 句子讲解可视化增强（2026-09-19）

针对“讲解里只写语法名称，但看不出句中哪个词对应哪个成分”的问题，本版继续增强内置讲解：

- **语法结构**不再只显示抽象公式；新增“句中对应”，直接把原句拆成“主语 / 变位谓语 / 宾语从句 / 补语 / 状语”等片段。遇到 `que / qu’` 从句时，还会继续显示“从属连词 / 从句主语 / 从句变位谓语 / 从句补语或状语”。
- **时态 / 语气**新增“由哪些词体现”，把实际的变位词直接列出来。例如 `j’espère que tout va bien` 会指出 `espère` 和 `va`，而不是只写“直陈式现在时”。复合过去时、近期将来时和近期过去时会显示组成该时态的助动词/半助动词与实义动词。
- **语态**新增“判断依据”，会列出主要谓语，并说明主动/非被动判断或 `être + 过去分词` 的被动结构依据。
- **重点词语可点读**：讲解区域里的法语词改成带扬声器图标的按钮，点击后沿用原项目的单词点读/循环与词典逻辑。时态和语态证据词也可以直接点读。
- Service Worker 缓存键升级到 `fr2000-v9-20260919-grammar-evidence`。

以 `Bonsoir, j’espère que tout va bien.` 为例，结构区现在会明确显示：

- `Bonsoir` → 寒暄 / 插入语
- `j’` → 主语
- `espère` → 变位谓语
- `que tout va bien` → 宾语从句（主句补语）
- 从句再拆分：`que` → 从属连词，`tout` → 从句主语，`va` → 从句变位谓语，`bien` → 从句补语 / 状语

静态检查：内联 JavaScript 已通过 `node --check`，并对全部 2000 句执行讲解生成测试，未出现运行异常。
