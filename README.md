# 日常法语 4000 句 · 交流优先版

这是在原 `french-2000` 界面和交互基础上重做语料后的版本。页面布局、收藏、掌握标记、搜索、分类、分页、连续播放、单句循环、播放间隔、点词词典、内置讲解、PWA 等使用方式保持原样；核心数据从 2000 句扩展为 4000 句。

## 本版改了什么

- 20 个主题，每个主题 200 句，共 **4000 句**；ID 连续 1–4000。
- 删除了旧语料中“把两条独立寒暄机械拼成一句”的生成方式。例如不会再出现 `À bientôt, ravi de vous voir.` 这种语义互相冲突的组合。
- 语料围绕与法国人实际交流：问候礼貌、自我介绍、家庭、住房、作息、日期时间、天气、购物、餐厅、交通、酒店、问路、工作、学习、健康、数码、休闲、邀约、情绪、求助等。
- 中文释义与新句子同步；全部 **4000 句**都带有预生成的离线语法标注，包括结构、时态/语气、语态、判断依据、固定短语与可信度。讲解默认显示简要结论，也可展开详细分析。
- 点词词典同步扩充；当前内置点读索引为 **914 个**词形/词条，并显示本句义、原形、词性、性数/人称/时态等词形信息以及 1–3 条本语料例句。
- 常见多词表达按整体识别，例如 `s’il vous plaît`、`est-ce que`、`avoir besoin de`；短语可整体点读，不再只按单词机械拆解。
- 句子讲解采用可扫读的“句型结构 + 时态/语气”摘要卡；详细证据按需展开。手机词典采用底部抽屉，并提供当前词高亮、循环朗读/停止按钮和可播放的语料例句。
- 查词时会显示单词或整块短语的 IPA、节奏、连读/省音、拼写对应和关键音提示，并可用 0.75× 慢速听辨；这些发音资料同样可离线使用。
- 新增独立的 **字母与音标** 栏：26 个法语字母名称、37 个 IPA 参考音（12 口元音、4 鼻元音、18 辅音、3 半元音），附中文发音要领、常见拼写和可点读例词。
- 新增 **大屏滑动模式**：句子、字母、音标均可全屏逐条学习，上下滑切换，当前条目持续循环，支持默认详解开关、隐藏译文、倍速与收藏标记。
- 项目内置了 3 路法国法语发音；句子音频会裁掉首尾静音，并将异常过长的句中静音缩短为约 80ms 的短停顿。
- 手机底栏会显示当前播放内容，句子卡片提供明确的播放/停止按钮；搜索、分页、深浅色主题与键盘操作也已针对手机和无障碍使用优化。

## 大屏滑动模式

点击顶部或句子卡片／字母音标详解里的“↕ 大屏滑动模式”，从当前内容进入。

- 每屏一条；上滑下一条、下滑上一条。电脑可用上下方向键或滚轮；空格暂停／继续，Esc 退出。底部只保留暂停／继续与详解按钮，音色和倍速另行选择。
- 本浏览器第一次进入时，会浮现“上下滑动切换条目”提示，**1.5 秒**后自动消失，不拦截滑动操作；之后进入或刷新不会反复提示。开启系统减少动态效果时仍显示相同时长，但不播放动画。
- 默认使用当前分类、搜索或收藏筛选结果，音标使用当前分组；勾选“全部 4000 句”可扩大句子范围。顶部可切换句子、字母、音标。
- 默认当前条目**无限循环**，不受普通模式“单句重复次数”限制。速度、间隔、句子播放组合和音色沿用播放设置，也可在模式内选择音色和倍速。切换条目立即停止旧音频。
- 底部音色旁提供 **0.50×–2.00× 倍速选择**，步长 **0.05×**（31 档），与普通播放速度同步并记住设置。内置音频即时变速，不重播、不增加计数；系统朗读从下一段开始使用新速度。已移除重复的 0.75× 慢读按钮，需要时直接选 0.75×。
- 三音色交替只在完整播放成功后推进；加载失败或中途暂停不会跳过该音色。内置音频复用同一个播放器以保留 Safari 的元素播放许可；临时加载失败最多自动重试两次，停止或切句会取消重试。浏览器拒绝自动播放时单独提示手动继续，不误报为网络问题。
- 句子模式点击显示 **“手动切换／自动切换”** 的设置长条，可展开详细设置；收起时概括当前模式、遍数、区间及末尾停止／循环。支持每句播放 **1–100 遍**后自动切下一句。点击“应用并开始”生效；一遍指完整的播放组合（如“法语→中文→法语”），不是其中一段。暂停保留已完成遍数；手动切换句子会重新计数。
- 勾选“指定句子区间”可按全库编号选择起止句（含两端），覆盖原分类／搜索范围，并从起始句开始。不指定区间则使用当前列表；到末尾默认停止，也可勾选“到末尾循环回第一条”。次数和区间设置会记住，字母／音标仍保持单条循环。
- 右上角 **“锁定”** 开启防误触：只保留上下滑动（电脑可用垂直滚轮）与“解锁”，禁用其他按钮、例词点读、译文揭示及页面快捷键；详解区域的上下滑动也直接切条目。播放和自动切换不受影响，解锁恢复控制。重新进入或刷新不会自动上锁；这不是系统锁屏，浏览器自身的返回／关闭操作仍由浏览器控制。
- “默认显示详解”会记住偏好，并应用于每条新内容；底部“显示／收起详解”只影响当前条目。长详解可单独滚动，滚到边缘后再滑一次才切换条目。
- 展开 **“字号与离线”**，可分别调整法语／字母、中文译文、音标大小，范围为默认字号的 **80%–160%**，步长 **5%**。即时生效、不打断播放，刷新后保留，并可一键恢复默认；只调整大屏模式正文，普通列表和详解字号不变。
- 同一面板可按编号下载句子区间的 **三种内置法语音色**；也可点“使用学习区间”沿用已应用的指定区间。显示就绪文件数，跳过已有缓存，支持取消及再次下载补齐；不会改变当前播放区间。仅在句子模式显示下载区，字母／音标仍使用原栏目下载按钮。
- 离线下载需要 HTTPS（本地开发可用 localhost）及浏览器缓存支持。建议使用 Wi-Fi 并保持网页打开，退出大屏模式不取消下载，离开／刷新页面会尝试取消；重开后再次下载即可跳过已有文件。网络失败、超时、存储空间不足会明确提示，不能保证系统后台持续下载。下载包不包含系统中文朗读；浏览器清理缓存后需重新下载。
- “先隐藏译文”支持听音回忆，点一下揭示答案，切换条目后重新隐藏。句子可直接收藏、标记掌握；字母音标可点读例词，播完返回原来的循环。
- 切到后台会暂停，返回后点“继续”；退出模式会停止音频并恢复原页面。浏览器允许时进入原生全屏，否则采用铺满网页可视区域的界面。
- 界面支持离线使用；音频需预先缓存，字母／音标可在对应栏目一键下载。离线时请选择内置音色，系统语音和中文朗读取决于设备支持。

## 本地生成全部内置语音

需要：Linux / WSL、Python 3、`python3-venv`、`ffmpeg`，以及可访问 Microsoft Edge TTS 服务的网络。

在项目根目录执行：

```bash
bash build-french-4000-neural.sh "$PWD"
```

脚本会生成：

- 4000 个句子 × 3 个音色
- 914 个点读词条 × 3 个音色
- 合计 **14,742 个 MP3**

固定使用三个 `fr-FR` 法国法语单语神经音色：

- `fr-FR-HenriNeural`
- `fr-FR-EloiseNeural`
- `fr-FR-DeniseNeural`

脚本开始时会在线读取音色清单，并验证三个音色的 `Locale` 必须等于 `fr-FR`；不使用 Multilingual 音色，也不自动切换语言。这样能显著降低 `Wi-Fi`、`Bluetooth`、`parking` 等拼写像英语的词被按英语口音朗读的风险。

### 静音处理

每个 TTS 原始 MP3 生成后立即经过 `ffmpeg`：

1. 完全裁掉**开头和结尾**的静音，不再额外添加保护留白；
2. 保留标点处的短停顿，但把异常过长的句中静音缩短到约 **80 ms**；
3. 可用 `FRENCH_MAX_SENT_PAUSE` 调整句中静音上限（单位为秒，有效范围 0.04–1.0）。

因此页面把“句子间隔”设为 `0s` 时，不会再叠加 TTS 文件本身常见的长首尾空白。

例如把句中静音上限设为 60 ms：

```bash
FRENCH_MAX_SENT_PAUSE=0.06 bash build-french-4000-neural.sh "$PWD"
```

脚本支持断点续跑：已经存在且大小正常的目标 MP3 会跳过。全部成功后会把 `audio/voice-manifest.json` 的 `availableSlots` 写为 `[1,2,3]`。

## 字母与音标

点击页面顶部的“字母与音标”，再切换“26 个字母”或“法语音标”。

- 点卡片开始点读，再点当前卡片或“停止”即可结束；重复次数、速度和间隔沿用现有设置。
- 支持 **0.75× 慢读**、例词单次播放、当前分组顺序听一遍。左右键／底栏上一项、下一项也可切换发音条目。
- 字母名称和例词使用所选的系统音色，或 Henri / Eloise / Denise 三路内置音色，支持三音色交替。
- **IPA 使用固定音色的真人语音参考录音**，不把符号交给 TTS 朗读。部分辅音录音含辅助元音，页面会提示；法语实际发音可再听例词。
- `/ɑ/` 与 `/a/`、`/œ̃/` 与 `/ɛ̃/` 的合并和借词 `/ŋ/` 有特别说明。`/œ̃/` 参考录音用比利时法语的 `un`，它本身只有一个鼻元音；不能把现代法国各地口音说成只有唯一实现。
- 手机端点卡片打开讲解抽屉，可点遮罩、关闭按钮或 Escape 退出并停音。不会把字母学习计入 4000 句的学习进度。
- 点击 **下载本栏离线音频**，完整缓存本栏 **232 个 MP3**（26 字母 × 3 音色 + 39 例词 × 3 音色 + 37 参考音）。断网时选择内置音色；系统语音是否离线取决于设备。
- PWA 更新保留已缓存的句子音频；MP3 支持离线 Range 请求。`file://` 模式直接读取项目内音频，不使用 Service Worker 下载。

重新生成（不会改动原句子音频）：

```bash
python -m pip install edge-tts
python build_phonetics_audio.py
python build_phonetics_audio.py --check
```

需要 `ffmpeg`、`curl` 和网络。脚本可断点续跑；Wikimedia 限流时遵守服务器的重试间隔。参考源与 SHA-1 固定在 `phonetics-sources.js`；仅在核对源文件和署名后使用 `--refresh-sources` 更新。

`audio/phonetics/ipa/` 中的录音来自 Wikimedia Commons，分别采用 CC BY-SA 3.0、CC BY-SA 4.0 或 CC0；原作者、原文件、许可链接及转换说明逐项列于 `phonetics-sources.js` 和页面“使用说明与录音来源”。转换后的音频保留其各自许可，不改变其他项目文件的许可。录音仅转换为单声道 MP3 并裁首尾静音，没有额外添加留白，保留参考录音内部用于展示发音的停顿。

浏览器回归测试：

```bash
python -m pip install playwright
python -m playwright install chromium
python tests/test_phonetics.py
python tests/test_swipe_study.py
python tests/test_swipe_auto_lock.py
python tests/test_swipe_speed_audio.py
python tests/test_large_screen_ui.py
python tests/test_swipe_fonts_download.py
```

测试使用临时本地服务器和独立浏览器数据，检查实际 MP3 播放、次数/音色/慢读、失败提示、停止互斥、手机抽屉、键盘焦点、子路径部署、缓存升级、232 音频下载、断网重载与音频范围请求。Linux 需要 Chromium 对应系统依赖。

滑屏测试另外检查真实触摸手势、详解滚动边界、无限循环、快速切换时的旧请求取消、隐藏译文、偏好保存、原生全屏与回退、滚轮／键盘、不同屏幕尺寸和后台暂停。

大屏界面测试检查统一命名、精简底部按钮、首次提示的 1.5 秒时长／跨刷新保存／提前退出清理、减少动态效果，以及设置长条的展开收起、概括内容和不同屏幕尺寸。

字号与下载测试检查三类独立字号及重置／保存、不同内容和屏幕尺寸，以及真实 Service Worker 区间下载、已有缓存跳过、失败重试、取消保留、存储配额错误、断网重载播放和 MP3 字节范围请求。

自动切换／锁定测试包含真实 MP3 的精确遍数、区间首尾与单句区间、完整中法播放组合计数、暂停续播、非法设置、旧回调取消、锁定防误触、真实手势及原生全屏退出后的锁定状态。

倍速／加载恢复测试覆盖 31 档速度、即时变速、偏好保存、三音色首次失败后的同音色重试、永久失败上限、权限错误提示及取消重试；另模拟按媒体元素授予播放许可的限制。该模拟不等同于 iPhone Safari 真机测试。

## 语料与词典源文件

- `sentences-4000.json`：4000 句法语、中文、IPA、分类与 ID。
- `extra-words-4000.json`：新语料补充词典。
- `build_corpus_4000.py`：重建语料和 IPA 的离线脚本。
- `build_extra_dictionary.py`：检查/生成新增词典数据。
- `learning-data-4000.js`：浏览器直接加载的静态句法、词形、短语与语料例句索引；支持 `file://` 和 PWA 离线使用。
- `build_learning_data.py`：重新生成上述学习数据；仅构建时需要 spaCy 与 `fr_core_news_sm`，网页运行时不需要 Python 或网络。
- `pronunciation-data-4000.js`：141 个已识别短语的离线 IPA 数据。
- `build_pronunciation_data.py`：重新生成短语 IPA；仅构建时需要 `phonemizer` 与 `espeakng-loader`。
- `phonetics-data.js` / `phonetics.js` / `phonetics.css`：字母与音标的离线教学内容、播放器交互和适配样式。
- `swipe-study.js` / `swipe-study.css`：大屏滑动模式、逐条循环播放、手势与全屏适配。
- `phonetics-sources.js` / `build_phonetics_audio.py`：参考录音署名清单与本栏三音色／参考音构建脚本。
- `apply_4000_update.py`：把语料和词典同步写回 `index.html`。
- `build-french-4000-neural.sh`：本地生成三路纯 `fr-FR` 内置音频，并裁首尾静音。

## 说明

本项目仍是学习材料，不把模板数量当成质量指标。新增句子按具体交际场景约束生成，并进行了重复、数量、词典覆盖和明显机械拼接的静态检查。IPA 用离线法语 eSpeak 生成，主要用于页面辅助显示；实际内置音频以 `fr-FR` Edge Neural TTS 为准。
