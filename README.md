...
- 项目内置了 3 路法国法语发音；句子音频会裁掉首尾静音，并将异常过长的句中静音缩短为约 300ms 的自然停顿。
...
2. 保留标点处的自然停顿，但把异常过长的句中静音缩短到约 **300 ms**；
3. 可用 `FRENCH_MAX_SENT_PAUSE` 调整句中静音上限（单位为秒，有效范围 0.1–1.0）。
...
```bash
FRENCH_MAX_SENT_PAUSE=0.25 bash build-french-4000-neural.sh "$PWD"
```
