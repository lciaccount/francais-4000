...
- 新增 **滑屏学习**：句子、字母、音标均可全屏逐条学习，上下滑切换，当前条目持续循环，支持默认详解开关、隐藏译文、慢读与收藏标记。
- 项目内置了 3 路法国法语发音；句子音频会裁掉首尾静音，并将异常过长的句中静音缩短为约 80ms 的短停顿。
- 手机底栏会显示当前播放内容，句子卡片提供明确的播放/停止按钮；搜索、分页、深浅色主题与键盘操作也已针对手机和无障碍使用优化。

## 滑屏学习

点击顶部“↕ 滑屏学习”，或句子卡片／字母音标详解里的“滑屏”按钮，从当前内容进入。

- 每屏一条；上滑下一条、下滑上一条。电脑可用上下方向键、滚轮或底部按钮；空格暂停／继续，Esc 退出。
- 默认使用当前分类、搜索或收藏筛选结果，音标使用当前分组；勾选“全部 4000 句”可扩大句子范围。顶部可切换句子、字母、音标。
- 默认当前条目**无限循环**，不受普通模式“单句重复次数”限制。速度、间隔、句子播放组合和音色沿用播放设置，也可在模式内选择音色或开启 0.75× 慢读。切换条目立即停止旧音频。
- 底部音色旁提供 **0.50×–2.00× 倍速选择**，步长 **0.05×**（31 档），与普通播放速度同步并记住设置。内置音频即时变速，不重播、不增加计数；系统朗读从下一段开始使用新速度。0.75× 慢读仍可一键开启／恢复。
- 三音色交替只在完整播放成功后推进；加载失败或中途暂停不会跳过该音色。内置音频复用同一个播放器以保留 Safari 的元素播放许可；临时加载失败最多自动重试两次，停止或切句会取消重试。浏览器拒绝自动播放时单独提示手动继续，不误报为网络问题。
- 句子模式展开 **“自动切换与区间”**，可设置每句播放 **1–100 遍**后自动切下一句。点击“应用并开始”生效；一遍指完整的播放组合（如“法语→中文→法语”），不是其中一段。暂停保留已完成遍数；手动切换句子会重新计数。
...
- `swipe-study.js` / `swipe-study.css`：滑屏学习、逐条循环播放、手势与全屏适配。
